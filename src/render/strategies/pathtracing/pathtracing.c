...
/*   Updated: 2024/11/20 19:05:26 by cfidalgo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_color *throughput, t_color *final_color)
{
	...
	if (!hit_record->figure)
		return (get_ambient_light(render->scene.ambient_light,
				&render->scene.settings.sky_color, throughput, final_color));
	...
	if (hit_record->figure->material.type == EMISSIVE)
		...
		sample_lights(render, &render->scene, hit_record, &light_color);
	...
	uint32_t *seed)
{
	...
	while (i < render->max_depth)
	{
		...
		process_lighting(render, &hit_record, &throughput, &depth_color);
